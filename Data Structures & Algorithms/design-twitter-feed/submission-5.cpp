...
public:
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        if (followee_cnt > 10) {
            priority_queue<pair<int, int>, vector<pair<int, int>>> latest_10;
            int earliest_of_latest_10 = INT_MAX;
            for (int followee: followee_list) {
                ...
                if (followee_tweets.size()) {
                    ...
                    earliest_of_latest_10 = min(earliest_of_latest_10, followee_tweets.back().first);
                }
                if (latest_10.size() >= 10) break;
            }

            for (int followee: followee_list) {
                const auto& followee_tweets = this->user_tweets[followee];
                if (followee_tweets.size() && followee_tweets.back().first < earliest_of_latest_10) continue;
